...
namespace Physics.Tests
{
    public class RaycastTests : PhysicsSystemsTests
    {
        ...
        public unsafe void RaycastAgainstStaticFloor()
        {
            ...
            Simulator.Broadcast(new RaycastRequest(world, origin, direction, new(&HitCallback), distance, (ulong)temp.Address));
        ...
        public unsafe void RaycastWithResults()
        {
            ...
            Simulator.Broadcast(new RaycastRequest(world, Vector3.Zero, Vector3.UnitZ, new(&HitCallback), 100f, 0));
            Assert.That(FindProof(0), Is.True);

            cubeTransform.LocalPosition = new(0, 0, 10);
            Simulator.Update();

            Simulator.Broadcast(new RaycastRequest(world, Vector3.Zero, Vector3.UnitZ, new(&HitCallback), 100f, 1));
            Assert.That(FindProof(1), Is.True);

            cubeTransform.LocalPosition = new(5, 0, 0);
            Simulator.Update();

            Simulator.Broadcast(new RaycastRequest(world, Vector3.Zero, Vector3.UnitZ, new(&HitCallback), 100f, 2));
